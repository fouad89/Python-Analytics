...
#EX 73 Create a script that reads this file, multiplies its values by two and saves the output in a new text file.
import pandas as pd
...
with open('sampledata_x_2.txt') as file:
...
# EX 78. genereate a password of length 6
import random
...
# EX 79. generate a password with specific criteria
'''Create a program asks the user to enter a new password and check that the submitted password
should contain at least one number, one uppercase letter and at least 5 characters. If the conditions are generated,
